...
export class IconObject extends DrawnObjectBase {
    ...
    protected _resize() {
        //=== YOUR CODE HERE ===
        if (this.resizesImage || !this._image?.canvasImage) return;
    ...
    protected override _drawSelfOnly(ctx: DrawContext): void {
        ...
        if (this.resizesImage) {
            ...
            //=== YOUR CODE HERE ===
            ctx.drawImage(this.image.canvasImage, 0, 0, this.image.canvasImage.naturalWidth,
                this.image.canvasImage.naturalHeight)
        }
    }
